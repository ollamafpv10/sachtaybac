...
function calculateTongDatMoi(book) {
    const mang = parseFloat(book.mang) || 0;
...
function updateSummary() {
    totalRowsSpan.textContent = books.length;
    const totalAmount = books.reduce((sum, book) => sum + calculateTongPhaiDat(book), 0);
...
function renderTable() {
    ...
    tbody.innerHTML = books.map((book, index) => createTableRow(book, index)).join('');
